...
class FlinkTableBuilderTest(unittest.TestCase):
    ...
    def test_window_agg_transform_without_window_size_and_limit(self):
        df = self.input_data.copy()
        source = self._create_file_source(df)
    ...
    def test_window_agg_transform_with_window_size(self):
        df = pd.DataFrame(
            [
    ...
    def test_window_agg_with_window_size_and_limit(self):
        df = pd.DataFrame(
            [
    ...
    def test_window_agg_first_last_value(self):
        df = self.input_data.copy()
        source = self._create_file_source(df)
    ...
    def test_window_agg_row_num(self):
        df = self.input_data.copy()
        source = self._create_file_source(df)
    ...
    def test_window_agg_filter_expr(self):
        df = pd.DataFrame(
            [
    ...
    def test_window_agg_with_different_criteria(self):
        df = self.input_data.copy()
        source = self._create_file_source(df)
